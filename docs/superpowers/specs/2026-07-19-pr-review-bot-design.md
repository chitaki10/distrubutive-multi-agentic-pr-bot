...
**Post-Stage-8 bugfix (found live):** the contract gate's `max_len=20000` was originally applied uniformly to every step, including `fetch_diff` (step 1) — but a raw PR diff is not LLM-generated content and routinely exceeds 20,000 characters, so real PRs were getting `contract_rejected:too_long` on step 1 and failing outright. Caught by pushing a real PR and seeing `status='failed'`. Fixed by adding a `skip_length_check` flag (`RecordStepInput` → `record_state_version_activity` → `validate_agent_output(..., max_len=None)`), set only for the `fetch_diff` step; the empty/echo/error-shaped checks still apply to it. Re-verified against a second real PR: `fetch_diff` now records `status='ok'` with the full diff, and the run completes normally through all 5 steps.

## Dependency swap — `pybreaker` → vanilla Python circuit breaker (2026-07-21)

Stage 5's circuit breaker originally used the third-party `pybreaker` library (plus `tornado`, its undeclared internal dependency for async support, discovered and worked around during Stage 5's build). Replaced both with a ~40-line hand-rolled `CircuitBreaker` class in `src/prbot/circuit_breaker.py` (`CircuitBreakerError` exception, `call_async(fn, *args, **kwargs)`, closed/open/half-open state tracked via a failure count + `time.monotonic()` timestamp) — same semantics: closed and failing repeatedly opens the breaker after `fail_max` failures; an open breaker rejects calls immediately without invoking the wrapped function; after `reset_timeout` elapses it goes half-open and allows exactly one trial call, closing again on success or reopening immediately on failure. `agents/security.py`, `agents/style.py`, `agents/test_coverage.py` swap `import pybreaker` for `from prbot.circuit_breaker import CircuitBreaker, CircuitBreakerError` with no other logic changes. `pybreaker`/`tornado` dropped from `pyproject.toml`; verified by uninstalling both from the venv and re-running the full test suite (63/63 green) with neither package installed. New `tests/test_circuit_breaker.py` covers the closed→open→half-open→closed/reopen lifecycle directly, since this logic is now ours to maintain rather than a third party's.
